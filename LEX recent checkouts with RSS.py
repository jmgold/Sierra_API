...
#experiments with feedgenerator
def RSS_feed (table):
    feed = FeedGenerator()
    feed.title('Recent Checkouts At Lexington')
    feed.id('http://find.minlib.net')
    feed.link( href='http://find.minlib.net', rel='alternate' )
    feed.description('A list of recent checkouts')
    feed.language('en')
    for record in table["entries"]:
        feed_entry = feed.add_entry()
        feed_entry.id('http://find.minlib.net/iii/encore/record/C__Rb' + record["id"])
        feed_entry.title(record["title"])
        feed_entry.link(href="http://find.minlib.net/iii/encore/record/C__Rb" + record["id"])
    rssfeed = feed.rss_str(pretty=True)
    feed.rss_file('Lexington_rss.xml')
